...
: "${GLOBAL_STEP:=10}"
: "${MODEL_NAME:=qwen2p5-0p5b-instruct}"
: "${TONE:=default}"
: "${NUM_GPUS:=1}"

CKPT_FORMAT="models--mycollabllm--math-500--$MODEL_NAME--global_step_"
...
uv run --isolated --extra $INFERENCE_BACKEND -m skyrl_train.entrypoints.main_generate \
  ...
  trainer.run_name="collabllm_eval_multiturn_model_${MODEL_NAME}_global_${GLOBAL_STEP}_tone_${TONE}" \
  generator.rollout_log_path="$HOME/ckpts/rollout_logs/test_rollouts_collabllm_multiturn_${MODEL_NAME}_global_${GLOBAL_STEP}_tone_${TONE}_eval.jsonl"
